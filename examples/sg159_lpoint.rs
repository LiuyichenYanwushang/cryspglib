...
fn main() {
    ...
    let n_a = mag_ops
        ...
        .count();
    println!(
        "\nFull magnetic group: {} ops ({} unitary + {} antiunitary)",
        mag_ops.len(),
        n_u,
        n_a
    );
    ...
    let unitary_lg: Vec<usize> = lg_indices
        .iter()
        .filter(|&&i| !mag_ops[i].time_reversal)
        .copied()
        .collect();
    let antiunitary_lg: Vec<usize> = lg_indices
        .iter()
        .filter(|&&i| mag_ops[i].time_reversal)
        .copied()
        .collect();
    println!(
        "Magnetic little group at L: {} ops ({} unitary + {} antiunitary)",
        lg_indices.len(),
        unitary_lg.len(),
        antiunitary_lg.len()
    );
    ...
    for &i in &lg_indices {
        ...
        let r = op.rotation;
        println!(
            "  {:3} | [{:2},{:2},{:2};{:2},{:2},{:2};{:2},{:2},{:2}] | ({:4.1},{:4.1},{:4.1}) | {}",
            i,
            r[0][0],
            r[0][1],
            r[0][2],
    ...
    let l_irreps: Vec<&IrrepRecord> = irreps_of(h_sg)
        .iter()
    ...
    for ir in &l_irreps {
        if ir.spinor {
            ...
            println!(
                "  g_sg={} H_spin_count={} G_spin_count={}",
                g_sg,
                h_spin.0.len() / 9,
                g_spin.0.len() / 9
            );
            ...
            println!(
                "  {:6} spinor dim={} spin_lg={:?} n_lg={} spin_chars={:?} Wigner={:?}",
                ir.ml,
                ir.dim,
                indices,
                n_lg,

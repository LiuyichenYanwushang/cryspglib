...
fn mat_conj(m: &[Complex64]) -> Vec<Complex64> {
    m.iter().map(|c| c.conj()).collect()
}
...
fn consquare_root(m: &[Complex64], dim: usize) -> Option<Vec<Complex64>> {
    ...
    for _iter in 0..80 {
        ...
        for i in 0..n2 {
            u[i] = Complex64::new(
                (u[i].re + m_ucinv[i].re) * 0.5,
                (u[i].im + m_ucinv[i].im) * 0.5,
            );
        }
        let u_conj = mat_conj(&u);
...
/// Compute Type-A anti-unitary characters from matrices already reordered to
/// canonical H-operation order. This is used for the selected star-arm block,
/// whose dimension is smaller than the full ISO-IR induced matrix.
pub fn type_a_antiunitary_chars_high_dim_ordered(
    mag_seitz: &[SeitzOp],
    mag_lg_indices: &[usize],
    h_chars: &[f64],
    h_seitz: &[SeitzOp],
    ...
    let g0 = SeitzOp::new(a0.rot, a0.trans, false);
    let (g0_sq, _lattice_sq) = square_seitz(&g0);
    let a0sq = find_seitz(&g0_sq.rot, &g0_sq.trans, h_seitz)?;
    let a0sq_mat = &h_matrices[a0sq.op_index * block..(a0sq.op_index + 1) * block];
    let a0sq_complex: Vec<Complex64> = a0sq_mat
        .iter()
        .map(|&value| Complex64::new(value, 0.0))
        .collect();

    let is_minus_id = (0..h_dim).all(|row| {
        (0..h_dim).all(|col| {
            let expected = if row == col { -1.0 } else { 0.0 };
            (a0sq_mat[row * h_dim + col] - expected).abs() <= 1e-10
        })
    });
    if is_minus_id {
        return Some(vec![0.0; mag_lg_indices.len()]);
    }

    let u = consquare_root(&a0sq_complex, h_dim)?;
    let mut au_chars = vec![0.0; mag_lg_indices.len()];
    for (out_idx, &mag_idx) in mag_lg_indices.iter().enumerate() {
        if !mag_seitz[mag_idx].timerev {
            continue;
        }
        let operation = &mag_seitz[mag_idx];
        let spatial = SeitzOp::new(operation.rot, operation.trans, false);
        let Some(h_match) = find_seitz(&spatial.rot, &spatial.trans, h_seitz) else {
            continue;
        };
        let h_matrix = &h_matrices[h_match.op_index * block..(h_match.op_index + 1) * block];
        let mut trace = Complex64::new(0.0, 0.0);
        for row in 0..h_dim {
            for col in 0..h_dim {
                trace += u[row * h_dim + col] * Complex64::new(h_matrix[col * h_dim + row], 0.0);
            }
        }
        au_chars[out_idx] = trace.re;
    }
    Some(au_chars)
}

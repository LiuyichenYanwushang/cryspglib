...
下一轮的具体步骤（有限且不需要再猜格式）：
1. 造出立方点群及各子群类型（T、D2、C3、C2、C1 …）的特征标表（标准，可自造）；
2. 用「把首行的母群分解限制到子群」预测各上下文的行，与 `little_subduce` 的
   8 行匹配，得到行 ↔ 上下文的双射（并作为门禁：8 行必须全部匹配上）；
3. 对每个 (记录, w 源)：几何侧已有臂/轨道，取每个轨道的 `H ∩ 臂稳定子` 点群类型，
   查对应行的 pg=1 频率，按轨道求和；
4. 与 1,006 条记录 / 5,756 行存储频率逐条比对（含未列出者为 0 的负向核对）。

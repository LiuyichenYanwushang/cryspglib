...
* **B（2026-09-26 R6.5 更新）参数域已有精确划分；域内覆盖仍是采样**：R6.5 的普查
  ...
  **每个实数** `s` 成立，于是 `s ↦ [ω_s]` 是可除群 `(R,+)` 到有限群 `H²(P,U(1))` 的同态
  ...
  每域恒定，**每域一个代表点足以覆盖整个域**；非 coboundary 族只可能在例外参数出现。
  两条前提都在语料上核对（母群 341 个冻结操作精确固定方向、`generic_order` 同集合；
  子群侧门禁逐 (记录,标号) 数 `w_R = 0` 并与 generic 采样阶比较）。**被撤销的中间版本**：
  第一版把前提写成 `w_R ∈ L*`（成员资格），是错的——那种旋转确实一直在小余群里，但因子系
  只在孤立 `s` 上是 cocycle；实测见证 `w = (0,4,0)`、步长 `1/4`，当时 24,430 条 violation，
  改为精确固定后 exit 0。仍待：① 例外参数（小余群更大）的类只有逐点判定，而
  `MAX_ORDER = 8` 的目录对 1,980 个探针（子群阶 12/16/24/48）返回空、"空"同时意味着
  超范围与非 coboundary，需要阶无关的精确判定（M2.2）；② 域内恒定的数学结论还需要一次
  细网格重算作为引擎侧经验控制（M2.3）。作用域围栏：方向必须是所在群的倒格矢

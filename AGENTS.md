...
* **B（2026-09-26 R6.5 更新）参数域已有精确划分；域内覆盖仍是采样**：R6.5 的普查
  ...
  **每个实数** `s` 成立，于是 `s ↦ [ω_s]` 是可除群 `(R,+)` 到有限群 `H²(P,U(1))` 的同态
  ...
  10030 与 10038 均在 `t=1/8`），完整 full-star 分区见 R6.7 卡 3–6。非 coboundary 族只可能
  在**小余群**例外参数出现这一条仍成立。
  ...
  **仍待（按 §3b 的 R6.7 卡）**：① 覆盖全部折叠臂的完整分区（卡 3）与逐块统计（卡 4）；
  ② 跨参数比较必须先统一群身份与 gauge（卡 2、卡 5）；③ 域内重数恒定的引擎侧经验控制
  （卡 6，取代原 M2.3）。作用域围栏：方向必须是所在群的倒格矢
...
* **卡 1（更正数学声明）** 涉及 `subduction_line_domain.rs`、`docs/subduction-r6-coverage.md`、
...
* **卡 2（同群比较）** 涉及 `subduction_catalogue.rs`、`subduction_line_domain.rs`，必要时
...
* **卡 3（完整分区，本轮核心）** 涉及 `subduction_line_domain.rs`、

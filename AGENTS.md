...
* **B（2026-09-26 R6.5 更新）参数域已有精确划分；域内覆盖仍是采样**：R6.5 的普查
  ...
  **每个实数** `s` 成立，于是 `s ↦ [ω_s]` 是可除群 `(R,+)` 到有限群 `H²(P,U(1))` 的同态
  ...
  2,273/5,756 对严格大于参考分区；在其上的**引擎侧**重数/来源恒定控制见卡 5–6（census 目前
  仍按旧的参考分区取样）。非 coboundary 族只可能在**小余群**例外参数出现这一条仍成立。
...
* **卡 3（完整分区，本轮核心）✅ 已完成（本轮提交）** 涉及 `subduction_line_domain.rs`、
  ...
  **未做（留给后续卡）**：census 仍按旧的参考分区取样（卡 6 才接入 `full_star_partition` +
  `probe_parameters`）；区间内 `stored`/`constructed` 允许变化，跨参数比较走卡 5 的几何 + 字符
  匹配；母群形式边界仍由 `parent_domain` 提供。
* **卡 4（逐子群星、逐块统计）** 涉及 `line_domain_census.rs` 与必要的内部接口。每条统计
  必须同时绑定 **record / source / parameter / child-star block**，并含：块对应的母群臂集合、
  子群星大小与小余群、**该块** cocycle 是否平凡、**该块**目标来源、块维数/目标维数/重数。
  目标来源允许 `stored/constructed/mixed`，不得用"整个 probe 出现过构造目标"替代。恒等星
  识别继续用精确倒格等价检查，报告其参数变化时单列到达 Γ 的条件。永久回归：13688 的阶 16
  非平凡块属于 stored、不得被别的块的 constructed 污染；同一 probe 内不同块来源各自正确；
  增删无关块不改变已有块的 cocycle/source 分类。验收分两步：先在旧参考块口径下复现
  192/0/0 以确认归属错误消除，再按新完整分区对**全部块**重新清点（总数重测，不预设）。

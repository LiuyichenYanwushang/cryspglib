...
* **A. ✅ 参数语义已有 API 标记**：`LineSubduction::parameter_kind()` 通过精确母群倒格
  ...
  是上述 SG 196 见证。
* **B（2026-09-26 R6.5 更新）参数域已有精确划分；域内覆盖仍是采样**：R6.5 的普查
  （`cryspglib::irrep::subduction::star::line_domain` + `examples/line_domain_census.rs`）
  用精确有理算术枚举"某个操作进入 `k(t)` 稳定子"的参数：解集是 `Q` 的子群，由**一个
  精确有理步长**生成，`[0,1)` 内的例外参数是其有限残类（无浮点、无随分母增长的网格）。
  实测：73/73 源的例外参数**恰为 `{0, 1/2}`**（`t=0` 全点群、`t=1/2` 指数 3 子群；
  六种形状 `(12,1,4)/(12,2,4)/(24,2,8)/(24,4,8)/(48,4,16)/(48,8,16)`），冻结表操作数 ==
  该源 generic 稳定子阶（73/73，且旋转集合逐一相等）；子群侧折叠后
  ...
  **42,073 个 (记录, 参数) 探针、`unsupported=0`、`errors=0`**（`stored=33,985`、
  ...
  （decoy：方向 `(7,0,0)`、步长 `1/7`，1/24 与 1/120 上只剩 `t = 0`）。网格检查**也
  看不见错的帧**：第三轮审查把子群帧整体换成父群帧后门禁仍 exit 0，所以现在由**显式帧
  断言**把关——记录的子群号 == 嵌入的 subgroup、重建的子群倒格与嵌入的互为包含、子群
  旋转集合逐一相等、且每个旋转保持子群格不变；同一变异现在给出 4,054 条 violation、
  exit 1。真正算术独立的是**两种步长算法**（中心化扫描 vs `Lattice::coordinates` 映射）：
  母群 **2,580 次比较 / 0 分歧**（11 个 F 心立方母群），子群侧 **28,713 次比较 / 0 分歧**
  ——28,713 = Σ over 5,756 个 (记录,标号) 折叠方向 of |rotation_set(child_sg)|，覆盖
  **116 个子群 SG：P 89 / A 4 / C 16 / R 7**（倒格指数 1/2/3；**没有 I 心**，此前写错）。
  另有冻结表与 generic 稳定子的**旋转集合逐一相等**（73/73；模块测试与 example 门禁
  现在都断言集合，第一条只断言阶的版本已被第三轮审查判为声明不实）。`--sequential`
  ...
  **E2**（推导 + 独立重算）；**域内射影类已证明恒为平凡（M2，2026-09-26）**：设
  `P = {R : R^{-T}q1 = q1}`（精确固定折叠方向的旋转，`w_R = 0`），域内 `q(t)` 的小余群
  恰为 `P`；`ω_s(R,S) = χ_{s q1}(λ_RS)` 的 cocycle 恒等式只用到 `(R^T − I)q1 = 0`，故对
  **每个实数** `s` 成立，于是 `s ↦ [ω_s]` 是可除群 `(R,+)` 到有限群 `H²(P,U(1))` 的同态
  ⇒ 平凡 ⇒ `[ω_t] = 1`。剩下的 `t` 依赖只有 gauge（系数连续），重数是整值连续函数 ⇒
  每域恒定；**但"每域一个代表点足以覆盖整个域"已撤回（2026-09-26 外部审查，见 §3b）**：
  分区只由参考折叠方向生成，报告的分解还依赖每条臂自己的小余群与臂对合并（见证 ordinal
  ...
  **空条件**而非"更弱但成立"（作用域围栏 + 旋转保持倒格 ⇒ 每个旋转都满足：母群
  2,580/2,580、子群 28,713/28,713）；它既不蕴含"因子系对每个实数 `s` 成立"（在成员资格
  集合上 cocycle 恒等式于 51,804 个 (对, `s`) 探针中失败 4,877 个，而精确固定集合上
  51,804/51,804 成立），也不蕴含"`t·w_R ∈ L*` 对每个参数成立"（只在 `t ∈ sZ` 上成立）。
  此前引用的"24,430 条 violation"按描述不可复现（按描述是 5,117 对），一并撤销；见证
  `w = (0,4,0)`、步长 `1/4` 仍可复现。M2.2 已交付阶无关的精确类判定（见下 M2.2 条目）。
  **仍待（按 §3b 的 R6.7 卡）**：① 逐块统计（卡 4）；② 跨参数比较必须先统一群身份与 gauge
  （卡 5；同群比较的前提修复已在卡 2 完成）；③ 域内重数恒定的引擎侧经验控制（卡 6，取代
  原 M2.3）。覆盖全部折叠臂的完整分区已由卡 3 交付并常驻回归，见下条与 §3b。作用域围栏：方向必须是所在群的倒格矢
  （否则 `t` 与 `t+1` 是不同 k 点），否则返回 `ParameterDomainOutOfScope`；73/73 源与
  1,006/1,006 折叠方向都在域内。旧的"分母 3–12 采样"结论（下条）保留为历史。
* **B′. 分母 3–12 的全部非临界小分母样本已闭合（历史，R6.4）**：阶 8、coboundary
  D4/C4v 的二维标准表示已构造；`line_family_coverage --projective-sample-sweep` 对
  约分后分母 `3..12` 且 `t ∉ (1/4)Z` 的 42 个参数各计算 5,756/5,756 完整分解，
  `missing=0, other_errors=0, content_mismatches=0`；此前 54/30 条缺口全属这一族。
  旧的 `MAX_GRID` 分母悬崖已移除（一维投影字符按生成元阶枚举有限根，不随 cocycle
  分母增长；ordinal 10030 `DT1` 在 `t=1/1000000` 有端到端回归）。当时的边界是 solver
  `MAX_ORDER=8`、高维表只覆盖已证明的 C2×C2 / D3 / D4+coboundary 族；非 coboundary D4
  与更高阶小群仍显式失败（R6.5 的普查把这些边界换成了精确的域清单；域内 cocycle 类
  恒定仍未证明）。
...
> 每个结论都对应到实际检查的子群块。** 现有精确有理运算、分解引擎与整数 syzygy 求解器
> 继续复用；改动集中在参数分区、同调比较的前提检查、审计统计与跨参数比较。
> 触发它的三个已复现缺陷（外部审查给出见证，均已接受）：
> ① **P1 分区漏掉 full-star 变化**：`child_exceptional_parameters` 只对参考折叠方向枚举
> 例外参数。见证——ordinal 10030（SG 196 `DT1` → #18）参考候选只有 `{0,1/4,1/2,3/4}`，
> 但另一条折叠臂在 `t=1/8` 获得阶 4 **非平凡** cocycle；ordinal 10038（SG 196 `DT1` → P1）
> 所有子群小群恒平凡，臂却在 `t=1/8` 合并（`1/9`、`1/7` 六个一维块各重数 1，`1/8` 四个块
> 重数 2,1,1,2）。因此"当前分区内完整分解恒定、每域一个代表点足够"**已被证伪，待完整
> 分区后重新验证**（`subduction_line_domain.rs`、`docs/subduction-r6-coverage.md` §4b、
> 本文件 §2 B 的对应句子已按此标注撤回）。
> ② **P2 同调比较只比阶不比群**：`cohomologous_to` 仅检查阶相同就逐项相减。见证——
> SG 83、方向 `(0,3,1)`，`t=1/2`（C₂×C₂）与 `t=1/3`（C₄）返回 `Ok(true)`，商 cocycle
> 根本不是 cocycle。
...
全量重算/独立审核/提交与文档收口。** 完成标志：三个已复现问题都有永久回归；完整分区能
解释全部臂与星的变化；审计不再跨块混用证据；并且能准确陈述当前 73 个冻结参数族的覆盖范围。

* **卡 1（更正数学声明）✅ 已完成（`7729abd`）** 涉及 `subduction_line_domain.rs`、`docs/subduction-r6-coverage.md`、
  本文件。保存并辨认现有 `--domain-sweep` 草稿（**已存于分支 `wip/domain-sweep` 的
  `42ee4c5`，main 保持干净**）；把"当前分区内完整分解恒定、每域一个代表点足够"标为
  **待完整分区后验证**；更正 `w∈L* ⇒ t·w∈L*` 的错误解释。**保留并写明真正成立的局部
  定理**：设 `q(t)=t v`，固定小余群中每个旋转满足 `R^{-T}v=v`；对选定 Seitz 代表元
  `(R,τ_R)` 定义 `β_t(R)=exp(2πi t v·τ_R)`，则 `ω_t(R,S)=β_t(R)β_t(S)/β_t(RS)`
  ——即该固定小余群上的因子系统是 **coboundary**（对每个实数 `t`）。完整分解重数恒定
  还需补星、臂归属与表示输运的论证。验收：文档清楚区分"固定方向的局部定理"与
  "完整 full-star 分区的覆盖结论"，旧结论保留撤回说明。
...
* **卡 2（同群比较）✅ 已完成（`2f1e03f`）** 涉及 `subduction_catalogue.rs`、`subduction_line_domain.rs`，必要时
  ...
  操作对应；② 确认旋转集合相同并验证对应保持乘法；③ 按对应重排另一张 cocycle 表后逐项
  相减；④ 群不同返回明确错误（如 `IncompatibleLittleCoGroups`）。**同一旋转集合、代表元
  平移不同**可以产生相差 coboundary 的因子系统，不得仅因代表元不同而拒绝——要保证的是
  共同群元素身份与乘法。永久回归：SG 83 `(0,3,1)` `t=1/2` vs `1/3` 必须明确拒绝；同群
  同步重排结果不变；同群显式加 coboundary 返回同调；同群已知非平凡 vs 平凡返回不同调。
  验收：上述负例在修复前能失败；既有 syzygy 与 projective catalogue 测试继续通过。
  **结果**：`cohomologous_to` 按精确旋转建立操作对应并验证其保持乘法，再按对应重排后相减；
  群不同（含阶相同、旋转集合不同）返回 `IncompatibleLittleCoGroups`；代表元平移不同仍允许。
  回归含 SG 83 `(0,3,1)` `1/2` vs `1/3` 的拒绝见证、同群重排、D16→D4 非平凡 ≠ 平凡；
  `--lib catalogue` 12 项通过。
* **卡 3（完整分区，本轮核心）✅ 已完成（本轮提交）** 涉及 `subduction_line_domain.rs`、
  `subduction_star_decompose.rs` 及必要的少量内部接口。复用生产路径 `line_arms` 的**未约化**
  方向臂与精确逆转置作用：把母群臂 `a_i` 折叠为 `v_i=T^T a_i`、`q_i(t)=t v_i`。分区同时覆盖：
  (a) 母群小群增强/形式诱导边界（保留现有 parent-domain 判据）；(b) 某条子群臂的小群增强
  `t(R_H^{-T}v_i − v_i) ∈ L_H*`；(c) 不同臂进入同一子群星或折到同一点
  `t(R_H^{-T}v_i − v_j) ∈ L_H*`。(b)(c) 可用统一的 `(i,j,R_H)` 枚举实现：`i=j` 含稳定子
  变化，`i≠j` 含臂合并。要求：复用现有精确步长求解器；**差向量为零时记为"整个参数域恒
  成立"，不得当作有限孤立边界**；检查**所有**折叠方向的周期条件（不能只查参考臂）；同一
  参数去重但保留触发的臂与旋转见证；数学边界与官方锚点 `1/4`、一般抽样点**分开存放**
  （抽样点不得自动成为边界）；几何上的 `t mod 1` 周期继续与表示标签 monodromy 分开。
  永久回归：10030 的完整分区必须含 `1/8` 且检查该点其他臂的阶 4 非平凡 cocycle；10038
  即使子群小群阶恒为 1 也必须含 `1/8`。旧的"参考方向恰为四分之一网格"测试可留，但须标明
  它只测单方向查询、不代表 full-star 分区。验收：每个新增边界都有可重算的精确见证，两个
  反例都被正确切开。
  **结果**：新 API `line_domain::{arm_images, FoldedArm, StarEventKind, StarEvent, StarBoundary,
  FullStarPartition, full_star_partition}` 与 `decompose::line_star_geometry`（生产几何的公开
  读数）；生产 `decompose::line_arms` 改为委托 `line_domain::arm_images`，分区与引擎共用同一份
  臂表。`(i, j, R_H)` 统一枚举，差向量为零记为 `permanent_*`（恒真，不是边界），边界保留
  `(kind, arm)` 有界见证 + 精确计数，`boundaries` 与 `probe_parameters(parent)`（并上母群
  形式边界）分开存放。语料实测（新回归 `the_full_star_partition_of_the_frozen_corpus_is_the_eighth_grid`，
  5,756 对全覆盖、并行 9 s）：分区**恰为八分之一网格**（46,048 = 8 × 5,756 条边界）、
  **2,273** 对严格大于参考分区（多 11,009 条边界）、50,226 条折叠臂**逐条**是子群倒格矢、
  枚举旋转集合 == 生产折叠拉回的旋转集合（1,006/1,006）、事件计数
  1,039,264 / 1,981,608 / 7,474,912、恒真 27,008 / 0 / 178,036。两个见证各有常驻回归（10030
  的 `1/8` 双臂阶 4 非平凡类；10038 的 `1/8` 四臂合并 + 生产几何 6 块→4 块 + 生产分解
  `6×(1,1)` → `(1,1),(1,1),(1,2),(1,2)`）。变异实验：子群旋转换成母群旋转被旋转集合断言抓住；
  只留 `i=j` 被语料回归与 10038 抓住；只留参考臂三个回归全部失败。
  **未做（留给后续卡）**：census 的**主探针集**仍按旧的参考分区取样（`probes=42073` 逐字未变，
  卡 6 才切换并接入区间控制；卡 4 已另起 full-star recount 通过程消费 `full_star_partition`）；
  区间内 `stored`/`constructed` 允许变化，跨参数比较走卡 5 的几何 + 字符匹配；母群形式边界仍由
  `parent_domain` 提供。
* **卡 4（逐子群星、逐块统计）✅ 已完成（本轮提交）** 涉及 `examples/line_domain_census.rs`
  （本轮唯一改动文件；另有 `subduction_star_decompose.rs` 增量：`FullStarBlock` 现在保存并暴露
  它的折叠点、代表点位置与 `arm_indices()`，即**跨参数稳定的块身份**）。`BlockSource{Stored,
  Constructed,Mixed}` + **只吃一条 target 切片**的纯分类器 `classify_block`（旧两值口径
  ...
  **实测**（`--gate --require-covered`，8 线程 45–55 s，exit 0）：`probes=42073 stored=33985
  ...
  非平凡类 `7,828/408/0`（阶 4/8/16 = 6,126/1,882/228）；**2,332 个探针内部块来源不一致**
  ...
  **Γ 到达单列**：23,024 个 (参数,臂) 条目，**23,024 全部落在 full-star 边界上，例外 0、
  ...
  的函数而不是源的函数。
  **见证**：13688 `SM1` @ `1/8` 的阶 16 非平凡块 stored 且携带参考点，同 probe 另一块阶 4
  非平凡 **constructed** ⇒ probe 口径 constructed、逐块口径 stored；10038 `DT1` `1/9` 六块
  各 1 臂 → `1/8` 四块 2,1,1,2；10030 `DT1` `1/8` 有阶 4 非平凡 stored 块、`1/9` 无
  ...
  **与规格的偏差（如实记录）**：该 example 原本**没有** `#[cfg(test)] mod tests`（规格假设有），
...
* **卡 7（全量重算、独立审核、提交与文档收口）** 每张卡先跑对应小范围回归；阶段验收按本文件
  ...
  `--example line_domain_census`、`cargo run --release -p cryspglib --example line_domain_census
  -- --gate --require-covered --domain-sweep`、
  `cargo run --release -p cryspglib --example line_family_coverage -- --projective-sample-sweep
  --gate`。保留按记录并行与确定性排序输出，并比较串行/并行一致。文档要报告：完整分区的边界与
  区间数、新增边界里小群增强与臂合并各占多少、实际检查的 probe/块/区间数量、全部块上的
  cocycle/source 分类、失败/未覆盖/跳过数量，以及**哪些是数学结论、哪些是运行验证**。
  审核拆成两项独立任务：**数学审核**（分区事件是否穷尽、同调比较前提、gauge 与跨参数目标
  匹配）与**审计审核**（块归属、计数守恒、错误路径与故障注入）；由对抗性 review agent 复现
  并处理发现，不设审核时长上限；通过后按任务边界提交，提交说明只记录实际执行过的验证。
...
* 新模块 `src/irrep/subduction_line_domain.rs`（`irrep::subduction::star::line_domain`）：
  ...
  解集是 `Q` 的子群；先求 `t·w ∈ Z^3` 的公分母步长，再用商 `Z^3/L*` 的指数
  （exponent，P=1、A/B/C/I/F=2、R=3）定出最小 `n`，步长为二者之积，`[0,1)` 内例外
  参数是其有限残类（第四轮把这里早先误写的"中心化格指数"改正：格**指数**是
  `det L*`，F 心为 4、其余 P=1 / A,B,C,I=2 / R=3，实测 230 群为
  `{1:149, 2:58, 3:7, 4:16}`；扫描界 6 要覆盖的是 exponent `{1:149, 2:74, 3:7}`，
  两条都由 `every_space_group_reciprocal_lattice_is_integral_with_small_exponent`
  断言）。
...
* **实测**：73 源 / 1,006 记录 / **42,073 探针**，`unsupported=0`、`errors=0`；例外参数
  ...
  `--gate` exit 0。
* **仍未证明**（此条已被下面的 M2 推翻，保留为历史）：域划分是精确的，但"同一域
  内部射影 cocycle 类恒定"没有证明；域内代表性由 42 点采样、每记录 `t = 1/7` 与审计
  锚点支撑。**M2（2026-09-26）已证明更强的结论：类在域内恒为单位类**，见 §2 B 与
  `docs/subduction-r6-coverage.md` §4b；该段其余内容（域划分、探针计数、门禁）不变。
* **第二轮对抗性审查（针对 `6e0fb4c` 的 9 处修复）判定 EFFECTIVE、无 P0**，并给出 10 条
  ...
  ② `6e0fb4c` 提交信息里的"非空转谓词 2,076"**不可复现**（此分解已被第三轮审查撤销，
  见下第三轮 ② 条；第四轮给出最终口径：`t = 1/2` 上 5,160 个谓词中 682 个 `w = 0` 恒真、
  **4,478 个依赖步长**（1,038 真 / 3,440 假）；可判错数是 4,478）。
...
* **第三轮对抗性审查（针对 `0c17232`）判定"部分有效"**：8 条修复有效并复现，3 条 P1 必须
  修，均已修并留档：
  ① 子群侧新增的两种步长算法**只对步长独立、对帧不独立**（两个算法吃同一个格参数）；
  审查者把子群帧整体换成父群帧后门禁仍 exit 0。现补**显式帧断言**（子群号、倒格互相
  包含、旋转集合相等、旋转保持格不变），并用同一变异验证会失败（4,054 violations）。
  教训：以后写"某检查能看见 X"之前，先做 X 的变异实验。
  ② 我给出的"非空转"更正值**又错了一次**（第四轮再纠正一次，现已定稿）：正确分解是
  `10,320 = 5,160 个 t=0 谓词（预测恒真，对步长无信息）+ 5,160 个 t=1/2 谓词`，后者中
  **682 个是 `w = 0` 恒真**（旋转本来就在 generic 稳定子内）、**4,478 个依赖步长**
  （1,038 预测真、3,440 预测假）；可判错谓词数是 **4,478**。两次错法分别是把 `t=0` 的
  5,160 与 `t=1/2` 的 1,720 重复计数、以及把 `w = 0` 的那 682 个算成依赖步长。
  ③ "模块测试与 example 门禁都断言集合"是**假的**——当时只有模块测试断言集合，example
  只比阶；现已在 `check_invariants` 里补上集合断言（73 源）。
  ④ 计数口径更正：子群侧 28,713 = Σ over 5,756 个折叠方向（不是 1,006 记录 × 旋转 =
  6,236）；116 个子群 SG 的中心化类型是 P 89 / A 4 / C 16 / R 7（**无 I**）。
  ⑤ 另外修正：`minimal_parameter_step` 的文档补上"残类规则只在 `w ∈ L*` 时成立"的
  前提（R 心 SG 148 的 `w=(2,0,0)` 是反例）；`child_candidates` 的错误文本不再出现空
  label（改为 `"folded direction"`）；删掉一处死绑定与一处重复 `checked_sub`；父群网格
  补 `checks == 0` 的空转守卫。
* **第四轮对抗性审查（针对 `5939e69`）**：三条 P1 修复全部复现有效（帧断言、非空转口径、
  example 的旋转集合断言），给出 4 条非阻断残余，全部接受并在本提交处理：
  ① 非空转数字第三次也是最后一次更正：`t = 1/2` 的 5,160 个谓词里 **682 个是
  `w = 0`**（旋转本来就在 generic 稳定子里，恒真且与步长无关），可判错的是 **4,478**
  （1,038 真 / 3,440 假）；`docs/subduction-r6-coverage.md` 与本文 §8 均已按此改写，
  三次错误口径都在原处留档。
  ② 前三轮被撤销的中间数字改为**原地撤销标记**（§8 第二轮 ② 指向第三轮 ②），不再让
  读者把中间口径当结论。
  ③ 中心化扫描的前提从"散文声明"升级为**显式 fail-closed 守卫**：`minimal_parameter_step`
  检查格含于 `Z^3`，否则返回 `DomainCensusInconsistent`；见证 `diag(1/2,1/2,1/2)` 配
  `w = (1,0,0)`（坐标路线给 `1/2`，两条算法都测）。**写这条守卫的测试时发现文档里
  早已写错的数字**：扫描界 6 要覆盖的是商 `Z^3/L*` 的 exponent（实测 `{1:149, 2:74,
  3:7}`），而格**指数** `det L*` 在 16 个 F 心群上是 **4**（实测 `{1:149, 2:58, 3:7,
  4:16}`），不是此前写的"≤3"；新测试
  `every_space_group_reciprocal_lattice_is_integral_with_small_exponent` 把两条都
  断死在 230/230 个空间群上。
  ④ 帧断言的**兄弟群局限**已写进 example 注释：116 个子群中 109 个存在"同倒格同旋转
  集合"的兄弟 SG，断言区分不了；这不影响结论，因为帧只通过这两个量被消费，断言要排除
  的正是会改变它们的帧。
* **第五轮对抗性审查（针对 `e93686b`，两名独立 reviewer：代码/控制 + 声明/账本）**：
  无 P0；两条功能声明复现（守卫载重、直方图独立重算），门禁计数逐字复现（`probes=42073
  stored=33985 constructed=8088 unsupported=0 errors=0`、网格 `371520/0` 与
  `4134672/0`、步长算法 `2580/0` 与 `28713/0`）。9 条发现全部接受，本提交处理：
  ① **P1：子群帧控制自指**。审查者把 `reciprocal_lattice` 对单个空间群改坏（SG 38 →
  另一个心、SG 38 → `Z^3`、SG 5 → `Z^3`），**门禁全部 exit 0 且计数逐字相同**——因为
  `record.child_sg == embedding.subgroup_sg()` 之后比较的是同一个函数同一个参数；阳性
  对照 SG 38 → `6Z^3` 以 54 条 probe error / exit 1 失败，证明该函数在关键路径上。现补
  两条控制：`probe_record` 把普查自己（`child_candidates` 内联构造的格）算出的子群阶带
  到每个探针上与 `reciprocal_lattice(child_sg)` 重算的阶逐点比较；新单元测试用每个空间
  群**自己的纯平移**导出同余条件并断言其条数 == 格指数（包含 + 同指数 ⇒ 相等）。
  **两个变异现在都被抓住**（见下 ③）。教训：`f(x) == f(x)` 形式的"帧断言"不是控制。
  ② **P1：出处过度声明**。`e93686b` 提交信息说错数字"写在 `minimal_parameter_step` 的
  文档、coverage 报告与 AGENTS.md 三处"，实测**只有两处**：coverage 报告（`0c17232`）
  写"格指数最大为 3"，AGENTS.md（`84172f6`）把 exponent 表当成"中心化格指数"（F 写成
  2）；`minimal_parameter_step` 的文档在 `e93686b` 之前**没有**这两条前提句，模块文档与
  `CENTRING_SCAN` 一直写的是 exponent。coverage 报告已按两条真实旧措辞改写，本文件
  §8 第二轮 ② 的措辞也一并更正；那条"第一版测试曾断言 index ≤ 3"的说法不可由 git 复核
  （草稿未提交），已从测试注释里删除，只留数学陈述。
  ③ **P2 六条**：`DomainCensusInconsistent` 变体文档仍写"四个 reason、只有一个可达、其余
  没有负例"，而新增的 `Z^3` 那条既可达也有负例 → 已改写为五个/两个可达，coverage 报告
  同步；`every_space_group_reciprocal_lattice_is_integral_with_small_exponent` 的消息
  曾说"16 个 index-4 就是 F 心群"但只查直方图 → 现在**逐号钉住**那 16 个空间群，并新增
  A/C 同余互斥见证（指数都是 2，此前对所有控制不可见）；`exponent <= 3` 断言与直方图
  重复，标注为 belt-and-braces 而非控制；`w = 0` 在守卫**之前**返回（零属于任何格，
  两条算法都给 `None`）已写进文档并加断言；守卫的 `reason` 字符串改为钉值匹配；
  `reciprocal_lattice` 的"works for any space group"改为"该空间群号**存储的 Hall
  setting**"（27 个号有不同心的 setting，24 个 `SG_DATA_HALL` 不是第一个 Hall）；
  兄弟群注释改为如实说明普查只通过倒格与旋转集合消费帧、而兄弟帧由
  `SubgroupEmbedding::build` 的 `StaleIsotropyRecord` 挡住，记录号一致性检查因此是复述。
  ④ **流程教训（写入流程本身）**：两名审查者共用同一个 checkout 与 target 目录，其中一位
  在做变异实验并重建，导致另一位看到**源码上不可能**的中间值（`reciprocal_lattice(38)`
...
* **M2.2（2026-09-26）阶无关的类判定**：见 `docs/subduction-r6-coverage.md` §4b。要点：
  ...
  35,039 探针 0 错误，非平凡只出现在阶 4（3,260）、阶 8（936）、阶 16（134）。
...
* **R6.7 卡 3 审核轮（数学审核，隔离 worktree + 独立 target，针对 `470ae2a`）**：**无 P0/P1**，
  4×P2；穷尽性与区间恒定推导**未被证伪**，并给出迄今最强的引擎侧证据：
  ① 独立重算（自写 `(i,j,R_H)` 枚举、步长走坐标路线、自建轨道闭合，**2,477,298 条关系**）
  与九个钉值逐项一致；每条差向量都在 `L*_H`（⇒ 残类列表就是解集，"非空区间 ⇒ w=0" 一步成立）；
  两种步长算法在**每一条关系**上一致（0 分歧）；46,048 次独立几何对照生产 `fold_arms`（0 不符）。
  ② **区间内引擎控制**：5,756 对 × 5 个区间内点 × 8 个区间 = **230,240 次区间内生产分解，
  0 错误、0 结构不符、0 呈现不符**（签名含块数、逐块星/臂/块/小群维数、排序目标与来源标志、
  `ParameterKind`）；区间内臂的精确稳定子阶最大 8，构造目录从不被要求给不支持的阶。
  ③ **存储 k 点巧合攻击**：逐臂 × 逐子群存储 k 点（±k）精确求解，**470,764 次巧合全部落在
  边界上、0 次严格落在区间内**（结构理由：区间内巧合 ⇒ 该 K 小余群 = 臂精确稳定子 ⇒ M2 使
  构造 cocycle 平凡 ⇒ 两种来源描述同一群同一类，只剩呈现差异）。
  ④ 两个见证的断言确为其实（10030 的 `1/8` 双臂阶 4 非平凡 + 参考臂平凡 + 引擎真变；
  10038 的 6 星→4 星与子项变化）。
  **4 条 P2（全部接受）**：① `intervals()` 等边界情形无测试（变异 (d) 改坏空分支与环绕分支 →
  439 passed 全绿）——已在 `36746f8` 补 `the_partition_accessors_cover_their_edges`；
  ② **见证上界与 `StarEvent::image` 无测试**（变异 (e) 去掉 `(kind,arm)` 上界并置 `image: 0`
  → 439 全绿；正常 ≤18/边界、≤120/记录，变异后 t=0 达 30–240）——已补：语料回归对每条边界
  断言 `witnesses.len() ≤ 3×臂数`、`(kind,arm)` 唯一、kind 与 `(arm,image,rotation)` 一致，
  并用 `assert_star_predicate` **逐条重算** `t(R^{-T}v_arm − v_image) ∈ L*_H`；
  `permanent_witnesses` 在 `t=0` 与 `1/7` 两点重算；③ 10038 测试里"每臂阶 1、类平凡"的
  循环是**空转**（P1 子群只有恒等旋转，两个量恒为常数）——已删除并写明理由，改为断言
  `child_rotations == {I}`；④ 措辞：`permanent_counts[1] == 0` 是 `arm_images` 精确去重的
  **必然推论**而非语料测量，`boundaries` 对真实记录也不可能为空（任何非零差向量的解集含
  `t = 0`）——已在断言消息与 `intervals()` 文档里注明。
  **更正**：变异 (a)（用母群旋转集合）实际被**两个**测试抓住（语料回归的旋转集合断言 +
  10038 的计数断言），卡 3 提交信息只写了前者——属"少写"，一并记录。
...
* **R6.7 卡 3（2026-09-26）完整 full-star 参数分区**：外部审查的 P1 修复。新 API
  ...
  `line_arms` 改为委托 `arm_images`（分区与引擎共用同一份未约化臂表；`MissingLineRotation`
  保留在薄包装里）。分区 = 对每条折叠臂 `v_i = T^T(R_i^{-T}v)`、每个有序臂对与每个子群旋转
  精确解 `t(R_H^{-T}v_i − v_j) ∈ L*_H`：`i=j` 小余群增强、`i≠j, R=1` 臂合并、`i≠j, R≠1`
  轨道识别；`w = 0` 记为 `permanent_*`（全域恒真，**不是**边界——这正是旧参考分区的镜像
  错误：参考方向被整个冻结小群精确固定，所以它自己的关系全是零）；每条边界保存**精确计数** +
  `(kind, arm)` 有界见证；`probe_parameters(parent)` 才把母群形式边界并进来；`intervals()`
  给出开区间（周期 `t mod 1`，与表示标签 monodromy 分开）。**实测**（新常驻回归
  `the_full_star_partition_of_the_frozen_corpus_is_the_eighth_grid`，1,006 记录 / 5,756 对
  全覆盖，rayon 并行 9 s）：分区**恰为八分之一网格**（46,048 条边界 = 8 × 5,756）、
  **2,273** 对严格大于参考分区（多 11,009 条边界）、50,226 条折叠臂**逐条**都是子群倒格矢
  （周期前提从参考方向扩到每条臂）、`rotation_set(child_sg)` == 生产折叠从嵌入拉回的旋转
  集合（1,006/1,006，0 不符）、参考候选 ⊆ 边界（35,039 条全过）；事件计数
  小余群增强 1,039,264 / 臂合并 1,981,608 / 轨道识别 7,474,912，恒真 27,008 / 0 / 178,036
  （**没有**两条臂的折叠方向完全重合）。**两个见证各成常驻回归**：10030（参考候选
  `{0,1/4,1/2,3/4}`）在 `1/8` 恰有两条 `(±4,0,0)` 臂获得阶 4 **非平凡**类、参考臂本身仍平凡；
  10038（P1 子群，参考候选只有 `{0}`）在 `1/8` 恰有 4 条合并事件、所有臂在 `1/8`/`1/9`/`1/7`
  的小余群阶皆为 1，生产几何 6 块 → 4 块（臂数 2,1,1,2），生产分解从 `6×(1,1)` 变为
  `(1,1),(1,1),(1,2),(1,2)`。**变异实验（先做后写）**：把枚举旋转换成母群旋转 → 新旋转集合
  断言抓住；只留 `i = j` → 语料回归与 10038 回归失败（10030 仍过：它靠"另一条臂的小余群
  增强"，两见证互补）；只留参考臂（= 旧分区）→ 三个新回归全部失败。**推导而非采样**：区间内
  没有旋转进入小余群 ⇒ 每条臂的旋转精确固定其方向 ⇒ M2 局部定理给出因子系处处 coboundary；

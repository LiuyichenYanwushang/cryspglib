...
- `--require-w-complete` 另外要求 5,756 条 `other_wave_vector_subduction` 也被计算。
  这在 pinned 归档下**不可能**：这些行引用的 73 个“别的波矢”irrep 在
  `data_irreps.txt` 里只有 `irrep_w_label` / `irrep_w_space_group` /
  `irrep_w_dimension` / `irrep_w_type` 四张表，既没有 k 矢量也没有特征标行
  （`isotropy_w_subduce_*` 只有 label 序号与频率），任何引擎都无法据此计算；该开关
  是这条独立问题的显式门禁，全表运行会退出 2 并打印
...
`other_wave_vector_subduction` 的 5,756 行——它们的 73 个源 irrep 在 pinned 归档
里没有 k 矢量与特征标行，任何引擎都无法据此计算；这条缺口由
`--require-w-complete` 门禁与 `w_scope` 行显式报告，不会被静默算作已完成。

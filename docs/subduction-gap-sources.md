...
- `scripts/iso_irrep_exact.py`：归档 ISO-IR PIR/CIR 源帧的严格加载器
  ...
  表示记录**（10,294 条：4,777 条离散 k + 5,517 条参数化 k 域），CIR 是另一套
  全表示记录（11,202 条）；两者不是"小群 vs 全群"的关系。
...
1. **PIR 归档同时含离散与参数化记录**：10,294 条 PIR 记录里 4,777 条是离散 k
   （参数槽全为零方向），5,517 条带自由参数（方向向量非零；1/2/3 个自由方向的
   条数为 7,739/5,268/8,482 是**槽**计数，同一记录可有多个槽）。域维数由**方向向量**
   决定，不能数参数槽。
...
3. **命中记录的"小群操作集"一致**：把每条命中记录的 `operations` 作为签名分组，
   ...
   按"自由方向最少"的**候选筛选**，否则会把一般位置的表示当成目标——正是任务卡
   警告的陷阱。注意这只按自由参数个数排序，**不等于**已完成小群最大性验证）。
...
其余统计：**322 组因子系统非平凡**、444 组含螺旋/滑移（nonsymmorphic）操作；
811 组存在"一般位置记录也穿过同一 q"的情形，已按"自由方向最少"的候选筛选剔除
（不筛就会把一般位置的表示当成目标；这只是候选筛选，不是小群最大性验证）；
...
1. **PIR 含离散与参数化记录**：离散记录是"三个零方向"的退化参数域（如 `GM1`
   常数 Γ、零方向）。域的维数由**方向向量**决定，不能数非空参数槽。
...
5. **帧契约（复核确认，勿再重复施加 U）**：冻结 setting 的 `U⁻¹` 已经进入引擎的
   ...
   `canonical_q` **已经**在归档帧里，再对它应用一次 U 是重复变换——上一轮报告里
   "U 尚未处理"的顾虑作废。R4 仍需处理的是操作代表元的格平移相位与 `child_shift`
   回退，不是坐标帧。
5. **因子系统**按任务卡公式用同一条乘法表算：SG 24 的 P 点 `(1/2,1/2,1/2)` 小余群阶 4、
   3 个带非格平移的螺旋操作、ω ∈ {1/4, 3/4}（I 心 + 三个 2₁ 螺旋），可作螺旋/滑移见证；
...
筛选候选（原 787）；444 组含螺旋/滑移操作（原 442）；星内各臂小余群阶全部一致
...
- **螺旋/滑移**：SG 24（I2₁2₁2₁，I 心 + 三条 2₁）：P 点小群 4 个操作、其中 3 个带
  非格（螺旋）平移、ω = 1/4 与 3/4（见单测 `test_screw_little_group_is_projective`）。
...
* 三类结果：215 解析（小余群阶 1，Bloch 相位路线）/ 684 参数化来源（归档 PIR 精确
  ...
  nonsymmorphic/projective 单列：322 组 ω 非平凡、444 组含螺旋/滑移操作。

**R3 边界之外（下一张卡 R4 的入口，本卡不实现）**：把 **684** 个来源候选在代入参数后
的字符/矩阵真正求值成可用的分导目标（归档矩阵块完整性已逐组确认：899/899 组完整），
并在有离散表可对照的 q 上交叉验证；同时处理操作代表元的格平移相位与既有的
`child_shift` 回退。**口径：684 只是来源候选数，不代表参数求值与目标表示完整性已
验证**。

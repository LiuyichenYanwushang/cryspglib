...
* **已知缺口（记账中，不写进"已支持"）**：母群倒格使 `k(t) ≡ -k(1/4)` 的**共轭参数**
  （`t = -1/4, 3/4, 7/4`）上，共轭不改变恒等重数，所以 `content(t)` 必须等于 pinned；
  实测 **199 行给出不同重数、58 行报非整数重数**（fail-closed），三者计数相同。根因是
  线源字符在共轭代表元上的相位约定（`t = 1/4, 5/4` 恰好落在同一正代表元上，所以全绿）。
  修复的验收是"199/58 → 0 且前两档保持全绿"，记账处是
  `line_family_coverage.rs` 的常量与 `target/r6_family.tsv`。

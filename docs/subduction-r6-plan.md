...
验收：覆盖说明里的每个数字都能由仓库内脚本/测试复算；四种证据级别分开列。交付见
[`subduction-r6-coverage.md`](subduction-r6-coverage.md)。该报告明确区分官方锚点、沿线
monodromy、几何证明与无外部 oracle 的一般参数内部一致性。

### R6.3 D4 参数样本缺口闭合（本轮增量）

新增阶 8 的 D4/C4v coboundary 字符目录：四个 gauge-twisted 一维 irrep 加唯一标准二维
irrep；非 coboundary D4 由真实 D16 中心商 cocycle 负例 fail-closed。完整字符表在 453 条
有可读冻结字符的 D4 子群记录上逐操作匹配（332 个一维来源、121 个二维来源），同时目录
总交叉检查为 2,661 行 / 22,302 个操作。`MAX_ORDER=8` 同时使阶 8 的阿贝尔 coboundary
小余群走通用一维路径。此前 `t=1/7,1/6,1/3,2/7` 的 54 行与 `t=3/8`
的 30 行缺口在 `line_family_coverage --projective-sample-sweep` 的 5,756 行全扫中均归零。
这关闭五个明确采样点，不声明其它 rational `t` 全覆盖；order >8 与非 coboundary D4
仍显式拒绝。

### R6.4 分母 3–12 的全部非临界小分母采样

继续对 `0<t<1` 中约分后分母 `3..12` 且 `t ∉ (1/4)Z` 的 42 个参数运行 `--projective-sample-sweep`：
每个参数均完整计算 5,756 行，缺失、其它错误与恒等重数 mismatch 都为零。三个 D4 端到端
见证在这 42 点上均构造出二维目标，并通过维数与逐操作重建检查。library 常驻回归直接保护
三个见证的 42 点完整路径；5,756 行 × 42 点的全表扫描保留为显式 CLI gate，约需 1–2 分钟，
不放进默认测试时长。此批次仍是有限采样，不能推出任意分母的
`t` 都完整支持。
